...
static int gp2a_opt_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	if(!proximity_enable)
	{
		config = GPIO_CFG(VIR_LED_EN, 0, GPIO_OUTPUT, GPIO_PULL_DOWN, GPIO_2MA);
		err = gpio_tlmm_config(config, GPIO_ENABLE);
		if (err) 
			printk(KERN_ERR "%s: gpio_tlmm_config(%#x)=%d\n", __func__, VIR_LED_EN, err);
		
		config = GPIO_CFG(GPIO_SENSE_OUT, 0, GPIO_INPUT, GPIO_PULL_UP, GPIO_2MA);
		err = gpio_tlmm_config(config, GPIO_ENABLE);
		if (err) 
			printk(KERN_ERR "%s: gpio_tlmm_config(%#x)=%d\n", __func__, GPIO_SENSE_OUT, err);
	}
...
static int gp2a_opt_resume(struct i2c_client *client)
{
	...
	if(!proximity_enable)
	{
		config = GPIO_CFG(VIR_LED_EN, 0, GPIO_OUTPUT, GPIO_PULL_UP, GPIO_2MA);
		err = gpio_tlmm_config(config, GPIO_ENABLE);
		if (err) 
			printk(KERN_ERR "%s: gpio_tlmm_config(%#x)=%d\n", __func__, VIR_LED_EN, err);
